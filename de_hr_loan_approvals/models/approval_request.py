    ...
    def action_refuse(self, approver=None):
        ...
        if approver_count == 0:
            ...
            if tot_refused_count ==  tot_approver_count:
                if self.loan_id:
                    self.loan_id.sudo().action_refuse()        
        ...
        for request in self:
            ...
            if status_lst:
                if status_lst.count('cancel'):
                    ...
                    status = 'approved'
                    if request.loan_id:
                        request.loan_id.sudo().action_approve()
                    
                elif status_lst.count('refused') >= minimal_approver:
                    status = 'refused'
                    if request.loan_id:
                        request.loan_id.sudo().action_refuse()    
                else:
                    status = 'pending'
            else:
                status = 'new'
...
class ApprovalApprover(models.Model):
    ...
    loan_id = fields.Many2one(related='request_id.loan_id')