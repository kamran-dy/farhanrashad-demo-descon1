...
class ApprovalRequest(models.Model):
    ...
    batch_id = fields.Many2one('hr.payslip.run', string='Batch Payslip', required=False)
    ...
    def action_refuse(self, approver=None):
        ...
        if approver_count == 0:
            ...
            if tot_refused_count ==  tot_approver_count:
                if self.batch_id:
                    self.batch_id.sudo().action_refuse()        
    ...
    def action_approve(self, approver=None):      
        ...
        if approver_count == 0:
            ...
            if tot_approver_count ==  tot_approved_count: 
                if self.batch_id:
                    self.batch_id.sudo().action_approve()       
        ...
        for request in self:
            ...
            if status_lst:
                if status_lst.count('cancel'):
                    ...
                    status = 'approved'
                    if request.batch_id:
                        request.batch_id.sudo().action_approve()
                    
                elif status_lst.count('refused') >= minimal_approver:
                    status = 'refused'
                    if request.batch_id:
                        request.batch_id.sudo().action_refuse()    
                else:
                    status = 'pending'
            else:
                status = 'new'
...
class ApprovalApprover(models.Model):
    ...
    batch_id = fields.Many2one(related='request_id.batch_id')