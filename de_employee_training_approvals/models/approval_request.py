...
class ApprovalRequest(models.Model):
    ...
    training_request_id = fields.Many2one('employee.request', string='Resignation', required=False)
    ...
    def action_refuse(self, approver=None):
        ...
        if approver_count == 0:
            ...
            if tot_refused_count ==  tot_approver_count:
                if self.training_request_id:
                    self.training_request_id.sudo().action_refused()        
    ...
    def action_approve(self, approver=None):      
        ...
        if approver_count == 0:
            ...
            if tot_approver_count ==  tot_approved_count: 
                if self.training_request_id:
                    self.training_request_id.sudo().action_approved()       
        ...
        for request in self:
            ...
            if status_lst:
                if status_lst.count('cancel'):
                    ...
                    status = 'approved'
                    if request.training_request_id:
                        request.training_request_id.sudo().action_approved()
                    
                elif status_lst.count('refused') >= minimal_approver:
                    status = 'refused'
                    if request.training_request_id:
                        request.training_request_id.sudo().action_refused()    
                else:
                    status = 'pending'
            else:
                status = 'new'
...
class ApprovalApprover(models.Model):
    ...
    training_request_id = fields.Many2one(related='request_id.training_request_id')