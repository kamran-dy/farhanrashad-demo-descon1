        ...
        if int(kw.get('employee_id')):
            ...
            if kw.get('description'):
                request_val = {
                    ...
                    'ref': kw.get('description'),
                    'payment_type': 'outbound',
                    'partner_type': 'supplier',
                }
                record = request.env['account.payment'].sudo().create(request_val)
...
class CustomerPortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if 'expense_adv_count' in counters:
            values['expense_adv_count'] = request.env['account.payment'].search_count([])
        return values
    ...
    def portal_my_expense_advs(self, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, search=None, search_in='content', groupby=None, **kw):
        ...
        searchbar_filters = {
            'all': {'label': _('All'), 'domain': [('state', 'in', ['draft', 'cancel','posted'])]},
        ...
        expense_adv_groups = request.env['account.payment'].search([])
        ...
        expense_adv_count = request.env['account.payment'].search_count(domain)
        ...
        _expense_adv = request.env['account.payment'].search(domain, order=order, limit=self._items_per_page, offset=pager['offset'])
    ...
    def portal_my_expense_adv(self, resignation_id, access_token=None, **kw):
        try:
            expense_adv_sudo = self._document_check_access('account.payment', resignation_id, access_token)
    ...
    def portal_my_next_advance_exp(self, expense_adv_id, access_token=None, **kw):
        ...
        id = expense_adv_id
        try:
            expense_adv_sudo = self._document_check_access('account.payment', next_next_id, access_token)
        except (AccessError, MissingError):
            return request.redirect('/my')
    ...
    def portal_my_pre_advance_exp(self, expense_adv_id, access_token=None, **kw):
        ...
        id = pre_pre_id
        try:
            expense_adv_sudo = self._document_check_access('account.payment', pre_pre_id, access_token)
        except (AccessError, MissingError):
            return request.redirect('/my')
