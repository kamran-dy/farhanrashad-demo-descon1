...
class ApprovalRequest(models.Model):
    ...
    rectification_id = fields.Many2one('hr.attendance.rectification', string='Attendance', required=False)
        ...
        if approver_count == 0:
            ...
            if tot_refused_count ==  tot_approver_count:
                if self.rectification_id:
                    self.rectification_id.sudo().action_refuse()        
    ...
    def action_approve(self, approver=None):      
        ...
        if approver_count == 0:
            ...
            if tot_approver_count ==  tot_approved_count: 
                if self.rectification_id:
                    self.rectification_id.sudo().action_approve()       
        ...
        for request in self:
            ...
            if status_lst:
                if status_lst.count('cancel'):
                    ...
                    status = 'approved'
                    if request.rectification_id:
                        request.rectification_id.sudo().action_approve()
                    
                elif status_lst.count('refused') >= minimal_approver:
                    status = 'refused'
                    if request.rectification_id:
                        request.rectification_id.sudo().action_refuse()    
                else:
                    status = 'pending'
            else:
                status = 'new'
...
class ApprovalApprover(models.Model):
    ...
    rectification_id = fields.Many2one(related='request_id.rectification_id')